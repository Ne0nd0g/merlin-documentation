...
.. note::
    Set the ``-proto`` flag to ``http`` or ``https` when using the ``winhttp`` client on Windows.

* ``go`` is the default HTTP client from Go's standard library
* ``winhttp`` is ONLY available on Windows and uses the `WinHTTP <https://docs.microsoft.com/en-us/windows/win32/winhttp/about-winhttp>`_ API
